#!/bin/bash
################################################################################
# lib/secrets.sh - Secret generation functions (Argon2 support)
################################################################################
...
# Device Encryption Key (32 bytes hex)
...
# Generate Argon2 hash (for test data)
...
generate_test_password_hashes() {
    ...
    log_success "Argon2 password hashes generated"
}
...
show_secrets_summary() {
    ...
    echo ""
    echo -e "${YELLOW}Full secrets saved to: $SECRETS_FILE${RESET}"
    echo ""
}
